...
foreach($vlan in $vlanlist) {
    Get-DnsServerResourceRecord -ZoneName '' -RRType 'A' | where-object {($_.Timestamp -lt [DateTime]::Now.AddMonths(-8)) -and ($_.RecordData.ipv4address -like $vlan)} # Enter ZoneName to query (e.g. domain.com)
}